...
public class WeatherIO implements IWeatherIO {
	...
	public void writeDataToFile(String filename, String topic, ArrayList<ITemperature> theWeatherList) throws IOException {
		...
		pWriter.append(topic); // Appends the topic to the file.
		pWriter.append("\n"); // Appends a new line to the file.
		
		for (ITemperature temp : theWeatherList)
		{
			pWriter.append(Math.round(temp.getTemperature(false) * 100.0) / 100.0 + "(C) " + Math.round(temp.getTemperature(true) * 100.0) / 100.0 + "(F)"); // Appends the temperature values to the file with the correct formatting.
			pWriter.append(", "); // Appends a comma to the file.
			pWriter.append(String.valueOf(temp.getYear())); // Appends the year to the file.
			pWriter.append(", "); // Appends a comma to the file.
			pWriter.append(temp.getMonth()); // Appends the month to the file.
			pWriter.append(", "); // Appends a comma to the file.
			pWriter.append(temp.getCountry()); // Appends the name of the country to the file.
			pWriter.append(", "); // Appends a comma to the file.
			pWriter.append(temp.getCountry3LetterCode()); // Appends the 3-Letter Code of the country to the file.
			pWriter.append("\n"); // Appends a new line to the file.
		}
