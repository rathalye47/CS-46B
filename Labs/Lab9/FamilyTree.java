...
public class FamilyTree 
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Can't find the colon. Please try again!");
		...
		else
		{
			...
			if (parentNode == null)
			{
				throw new TreeException("The parent node isn't found. Something is wrong in the data file.");
			}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Can't find the node with " + name1);
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Can't find the node with " + name2);
